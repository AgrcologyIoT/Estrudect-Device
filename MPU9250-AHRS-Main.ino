...
//          sprintf(myStr, "Counter-%dahmed%d", counter,155); 
        read_mpu9250_data();
        delay(100);
        doc["DevID"] = atoi(deviceID);  //convert astring to int
        //accelUpdate
        doc["AccelX"] = ax; // read accelX data sensor
        doc["AccelY"] = ay; // read accelY data sensor
        doc["AccelZ"] = az; // read accelZ data sensor      
       // update gyro. data sensor
       doc["GyroX"] = gx;// read gyroX data sensor
       doc["GyroY"] = gy;// read gyroY data sensor
       doc["GyroZ"] = gz;// read gyroZ data sensor
         // update mag. data sensor
       doc["MagX"] = mx; // read magX data sensor
       doc["MagY"] = my; // read magX data sensor
       doc["MagZ"] = mz; // read magX data sensor
       //read temperature
       doc["Temp"] = therm.object();//read temperature   
       serializeJson(doc, myStr); 
//       SerialUSB.println(doc);
